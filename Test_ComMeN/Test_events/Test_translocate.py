...
class TranslocateTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.event = Translocate(0.2, self.nodes, compartments[0], edge_class=Edge_Type1)
        self.event_not_number_edges = Translocate(0.3, self.nodes, compartments[0], edge_class=Edge_Type2,
                                                  rate_increases_with_edges=False)
        events = [self.event, self.event_not_number_edges]
    ...
    def test_rates(self):
        ...
        self.assertEqual(self.event.rate, 0.2 * (self.nodes[0]['a'] * 2 + self.nodes[1]['a'] * 1 +
                                                 self.nodes[2]['a'] * 0 + self.nodes[3]['a'] * 0 +
                                                 self.nodes[4]['a'] * 0))
        self.assertEqual(self.event_not_number_edges.rate, 0.3 * (self.nodes[0]['a'] * 1 + self.nodes[1]['a'] * 0 +
