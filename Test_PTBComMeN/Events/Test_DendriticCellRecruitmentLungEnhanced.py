...
class DendriticCellRecruitmentLungEnhancedBacteriaTestCase(unittest.TestCase):
    def setUp(self):
        compartments = [DENDRITIC_CELL_IMMATURE, BACTERIUM_FAST, BACTERIUM_SLOW]
    ...
    def test_rate(self):
        ...
        self.nodes[0].reset()
        self.assertEqual(self.event.rate, 0)
        self.nodes[0].update({BACTERIUM_FAST: 15, BACTERIUM_SLOW: 5})
...
class GetDendriticCellRecruitmentEventsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        compartments = [MACROPHAGE_REGULAR, BACTERIUM_SLOW, BACTERIUM_FAST]
        self.nodes = [LungPatch(0, compartments, 0.9, 0.3)]
        self.rate = 0.2
        self.half_sat = 20
