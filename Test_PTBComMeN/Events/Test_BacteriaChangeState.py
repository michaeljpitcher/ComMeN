...
class BacteriaChangeStateTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.event_f = BacteriaChangeByOxygen(0.1, self.nodes, BACTERIUM_EXTRACELLULAR_FAST, BACTERIUM_EXTRACELLULAR_SLOW, -1 * self.sigmoid,
                                              self.half_sat)
        self.event_s = BacteriaChangeByOxygen(0.2, self.nodes, BACTERIUM_EXTRACELLULAR_SLOW, BACTERIUM_EXTRACELLULAR_FAST, self.sigmoid,
                                              self.half_sat)
    ...
    def test_update(self):
        self.nodes[0].update({BACTERIUM_EXTRACELLULAR_SLOW: 2, BACTERIUM_EXTRACELLULAR_FAST: 3})
...
class GetBacteriaChangeEventsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sigmoid = 1.1
        self.half_sat = 1.2
        self.events = get_bacteria_change_events(self.nodes, {BACTERIUM_EXTRACELLULAR_FAST: 0.1, BACTERIUM_EXTRACELLULAR_SLOW: 0.2, SIGMOID:
                                                              self.sigmoid, HALF_SAT: self.half_sat})

    def test_events(self):
        self.assertEqual(len(self.events), 2)
        fast_event = next(e for e in self.events if e._compartment_from == BACTERIUM_EXTRACELLULAR_FAST)
        slow_event = next(e for e in self.events if e._compartment_from == BACTERIUM_EXTRACELLULAR_SLOW)

        self.assertEqual(fast_event._compartment_to, BACTERIUM_EXTRACELLULAR_SLOW)
        ...
        self.assertEqual(slow_event._compartment_to, BACTERIUM_EXTRACELLULAR_FAST)
